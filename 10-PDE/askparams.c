...
{
	return (options->term_iteration >= 0 && options->term_iteration <= MAX_ITERATION);
}
...
askParams (struct options* options, int argc, char** argv)
{
	...
	if (argc < 2)
	{
		...
		else if (options->termination == TERM_ITER)
		{
			do
			{
				...
				printf("Iterations> ");
				fflush(stdout);
				ret = scanf("%" SCNd32, &(options->term_iteration));
				while (getchar() != '\n');
			}
	...
	else
	{
		...
		{
			ret = sscanf(argv[6], "%" SCNd32, &(options->term_iteration));
			options->term_precision = 0;
