...
static
void*
calculate(void* thread_argument)
{
...
static
void
calculate_thread (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options)
{
    ...
    for (thread_id = 1; thread_id < options->number; thread_id++) {
		rc = pthread_create(&threads[thread_id].raw_thread, NULL, calculate, &threads[thread_id]);
		if (rc) {
...
main (int argc, char** argv)
{
	...
	calculate_thread(&arguments, &results, &options);
